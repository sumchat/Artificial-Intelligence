...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for layer in range(recur_layers):
        # Add recurrent layer
        simp_rnn = GRU(units, activation='relu',return_sequences=True, implementation=2,name='rnn'+str(layer))(input_layer)       
        input_layer = BatchNormalization(name='bn_rnn'+str(layer))(simp_rnn)     
    time_dense =  TimeDistributed(Dense(output_dim))(input_layer)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    conv_1d = Conv1D(filters, kernel_size, 
                     ...
                     activation='relu',
                     name='conv1d')(input_data)    
    #pooledlayer1 = MaxPooling1D(pool_size=4)(conv_1d)
    ...
    rnn = Bidirectional(GRU(units,  return_sequences=True),merge_mode="concat")(dropout1)
    # Add batch normalization to bidirectional rnn layer
    bnn = BatchNormalization(name='bn_rnn')(rnn)
    #TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bnn)

...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            node_pg = PgNode_a(action)
            if node_pg.prenodes.issubset(self.s_levels[level]):
                for node in self.s_levels[level]:
                    for prend in node_pg.prenodes:
                        if prend.symbol == node.symbol:
                            node_pg.parents.add(node)
                            node.children.add(node_pg)
                self.a_levels[level].add(node_pg)
    ...
    def add_literal_level(self, level):
        ...
        for node in self.a_levels[level - 1]:
            ...
            for s_node in node.effnodes:
                pgnode = PgNode_s(s_node.symbol,s_node.is_pos)
                pgnode.parents.add(s_node)
                s_node.children.add(pgnode)
                self.s_levels[level].add(pgnode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        inconsistent_eff = False
        for action1 in action_a1.effect_add:
            if action1 in action_a2.effect_rem:
                inconsistent_eff = True
        for action2 in action_a2.effect_add:
            if action2 in action_a1.effect_rem:
                inconsistent_eff = True
        return inconsistent_eff
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        inconsistent_eff = False
        for action1 in action_a1.effect_add:
            if action1 in action_a2.precond_neg:
                inconsistent_eff = True
        for action2 in action_a2.effect_add:
            if action2 in action_a1.precond_neg:
                inconsistent_eff = True
        for action1 in action_a1.effect_rem:
            if action1 in action_a2.precond_pos:
                inconsistent_eff = True
        for action2 in action_a2.effect_rem:
            if action2 in action_a1.precond_pos:
                inconsistent_eff = True
        return inconsistent_eff
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for parentnode1 in node_a1.parents:
            for parentnode2 in node_a2.parents:
                if parentnode1.is_mutex(parentnode2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent1 in node_s1.parents:
            for parent2 in node_s2.parents:
                if not parent1.is_mutex(parent2) or not parent2.is_mutex(parent1):
                    return False
        return True
